...
def clean(df_raw):
    ...
    # Normalize StockCode casing and surrounding whitespace. The raw
    # dataset has ~171 case-variant "duplicates" (e.g. "85123A" vs
    # "85123a") plus at least one whitespace-variant duplicate (e.g.
    # "47503J" vs "47503J ") that are the same product with identical
    # descriptions — just inconsistent data entry. Left unnormalized,
    # dim_product ends up with two distinct primary keys for one
    # product, which silently fragments its revenue across two rows in
    # any stock_code-level aggregation, and also collides with Power
    # BI's case-insensitive relationship-key comparison even though
    # Postgres treats them as valid distinct primary keys.
    df_raw = df_raw.copy()
